...
{
    public class ObjectReviewPresentationService
    {
            ...
            Func<YoloWorkerSmokeCandidate, string> buildCandidateDetail)
        {
            ...
            string summary = ObjectReviewPresenter.BuildSummary(objectCount);
            var rows = new List<WpfObjectReviewListItem>();

            if (objectCount == 0)
            {
                rows.Add(WpfObjectReviewListItem.Empty(ObjectReviewPresenter.EmptyText));
            ...
            for (int i = 0; i < manualSegmentCount; i++)
            {
                ...
                rows.Add(ObjectReviewPresenter.BuildManualItem(
                    manualRoiCount + i + 1,
                    className,
                    segment.Bounds,
                    shapeName,
            ...
            for (int i = 0; i < confirmedCount; i++)
            {
                ...
                rows.Add(ObjectReviewPresenter.BuildConfirmedItem(
                    candidate,
                    displayIndex,
                    bounds,
            ...
            int index)
        {
            ...
            return ObjectReviewPresenter.BuildManualItem(
                index + 1,
        ...
        {
            bool useIncremental = ObjectReviewSelectionService.ShouldUseIncrementalDelete(
                deletedSource,
            ...
            int selectedRowIndex = useIncremental
                ? ObjectReviewSelectionService.GetSelectionIndexAfterDelete(deletedObjectRowIndex, remainingObjectCount)
                : -1;
            ...
            return new WpfObjectReviewDeleteRefreshPlan(
                useIncremental,
                ObjectReviewPresenter.BuildSummary(remainingObjectCount),
                selectedRowIndex);
        }
    ...
    [Obsolete("Use ObjectReviewPresentationService.", false)]
    public sealed class WpfObjectReviewPresentationService : ObjectReviewPresentationService
    {
    }
