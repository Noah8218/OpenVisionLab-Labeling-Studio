...
namespace MvcVisionSystem
{
    ...
    public partial class WpfLabelingShellWindow
    {
        ...
        {
            if (isApplicationCloseApproved || externalAuditWorkflowService.IsExternalEvaluationDataAuditRunning)
            {
                return;
            }
            ...
            if (isApplicationCloseApproved || externalAuditWorkflowService.IsClosed || result.IsCanceled || !result.Started)
            {
                return;
            }
        ...
        {
            if (isApplicationCloseApproved || externalAuditWorkflowService.IsHistoricalSegmentationRemediationAuditRunning)
            {
                return;
            }
            ...
            HistoricalSegmentationRemediationAuditWorkflowResult result = await externalAuditWorkflowService.RunHistoricalRemediationAsync(
            ...
            if (result.Error != null)
            {
                SetModelStatus($"SEG \uBCF4\uC815 \uAC80\uD1A0 \uC2E4\uD328: {result.Error.Message}");
                AppendLog($"SEG remediation dry run failed: {result.Error.Message}");
                return;
            }
