...
- The U-Net valid-only diagnosis, weighted/crop experiments, and opt-in foreground selector are recorded. The selector improved unchanged full-frame valid macro Dice/IoU from `0.164849/0.097209` to `0.204437/0.127053`, but `contamination_spot` and `foreign_particle` remain zero-overlap, so it is not the default and held-out remains closed. Next is one unweighted CE plus foreground soft-Dice loss hypothesis on full-frame train/valid; do not mix crop or class weights. See `docs\UNET_E30_CLASS_CONFUSION_ANALYSIS_20260722.md`.
