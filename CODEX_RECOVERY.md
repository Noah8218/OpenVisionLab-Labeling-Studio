...
- U-Net's valid-only diagnostic and experiments are `docs/UNET_E30_CLASS_CONFUSION_ANALYSIS_20260722.md`. The internal selector improved unchanged full-frame valid macro Dice/IoU to `0.204437/0.127053`, but two classes still have zero overlap; it is opt-in only. Class weights and crop are not adopted, normal TCP training remains validation-loss based, and no held-out test was used. Next isolate unweighted CE plus foreground soft-Dice loss without crop or class weights.
