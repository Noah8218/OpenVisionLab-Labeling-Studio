...
산업용 비전 검사 데이터를 라벨링하고, 하나의 레시피 데이터를 모델별 포맷으로 변환해 여러 모델을 시험한 뒤, 학습된 모델로 다시 검사해보는 Windows 기반 작업 프로그램입니다.
...
OpenVisionLab Labeling Studio는 단순히 라벨 파일만 만드는 도구가 아니라, 로컬 Windows 환경에서 산업용 이미지 데이터셋을 준비하고 한 레시피의 라벨 데이터를 모델별 입력 형식으로 변환해 학습/검토/비교/적용하는 작업대입니다.

- `저장 라벨`과 `AI 후보`를 분리해서 봅니다.
- 학습이 끝난 `모델 후보`와 실제 검사에 쓰는 `현재 검사 모델`을 분리합니다.
- 객체탐지, 세그멘테이션, 이상탐지 목적을 같은 워크플로우 안에서 다룹니다.
- 하나의 레시피는 이미지, 클래스, 라벨, split, 근거를 가진 기준 데이터이며, 선택한 모델 어댑터가 그 데이터를 해당 모델 형식으로 변환합니다.
- 사용자는 라벨링의 의미와 데이터 품질의 영향을 배우고, 같은 근거 데이터에서 여러 모델의 품질·Takt·실패 사례를 비교합니다.
- GitHub의 모든 모델을 자동 지원한다고 주장하지 않습니다. 새 모델은 클래스/좌표/분할/결과 매핑과 focused 검증을 갖춘 어댑터가 추가된 뒤에만 지원합니다.
