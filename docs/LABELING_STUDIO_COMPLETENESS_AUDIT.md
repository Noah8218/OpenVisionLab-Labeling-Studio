...
2026-07-07: The image queue, AI-candidate left panel, saved-label left panel, and guide/tools left panel received focused compact-layout passes. The verified UI direction is to keep operator-critical state/actions visible while collapsing repeated explanatory text and first-run shortcut tiles by default, giving more room to the image list, object list, workflow content, and canvas. Covered by focused WPF tests and 1920x1080 visual-smoke captures; this is a layout/usability improvement only and does not change the product completeness ranking above.
