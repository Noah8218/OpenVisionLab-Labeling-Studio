...
## 2026-07-03 model runtime test/restart/stop presentation split

- Self-evaluation:
  - Model test, inference worker restart, and inference worker stop already had working execution paths, but failure/recovery text was still assembled directly in `WpfLabelingShellWindow.YoloEnvironmentRuntimeCommands.cs`.
  - This is the same area users read when they need to know whether inspection is running, failed, or needs a restart, so presentation text should not stay mixed with worker execution.
- Changes:
  - Added `WpfYoloEnvironmentRecoveryPresentation`.
  - Moved model-test mode-switch/start/completed/failure text and model-test recovery action into `WpfYoloEnvironmentCommandPresentationService`.
  - Moved inference-worker restart start/success/connection-failure/exception recovery text into `WpfYoloEnvironmentCommandPresentationService`.
  - Moved inference-worker stop start/completed/failure text into `WpfYoloEnvironmentCommandPresentationService`.
  - Kept `WpfLabelingShellWindow.YoloEnvironmentRuntimeCommands.cs` focused on runtime state checks, worker restart/stop calls, panel refresh, and adapting the returned presentation text to the UI.
  - Added `--wpf-labeling-shell` guards so these runtime command/recovery strings are generated by the presentation service and not reintroduced inline in the shell command file.
- Verification:
  - `dotnet build .\tests\LabelingApplication.Tests\LabelingApplication.Tests.csproj -c Debug /nr:false -m:1 /p:UseSharedCompilation=false /p:OutDir=artifacts\isolated-out\` passed with 0 warnings / 0 errors.
  - `dotnet .\tests\LabelingApplication.Tests\artifacts\isolated-out\LabelingApplication.Tests.dll --wpf-labeling-shell` passed.
  - `dotnet .\tests\LabelingApplication.Tests\artifacts\isolated-out\LabelingApplication.Tests.dll --wpf-yolo-model-settings-panel` passed.
  - `dotnet .\tests\LabelingApplication.Tests\artifacts\isolated-out\LabelingApplication.Tests.dll --runtime-command-failure-messages` passed.
  - `dotnet .\tests\LabelingApplication.Tests\artifacts\isolated-out\LabelingApplication.Tests.dll --python-model-runtime-connection` passed.
  - `dotnet .\tests\LabelingApplication.Tests\artifacts\isolated-out\LabelingApplication.Tests.dll --python-model-runtime-self-test` passed.
  - `dotnet .\tests\LabelingApplication.Tests\artifacts\isolated-out\LabelingApplication.Tests.dll --mvvm-infra` passed.
  - `dotnet .\tests\LabelingApplication.Tests\artifacts\isolated-out\LabelingApplication.Tests.dll --priority-workflow-docs` passed.
...
- 자체 평가:
  - YOLOv8/YOLO11 연결 흐름에서 Ultralytics 설치/제거는 사용자가 직접 실행 여부를 확인해야 하는 고위험 동작입니다.
  - 확인 팝업 제목/본문/버튼 문구와 최근 실행 결과 상세(`결과`, `대상`, `명령`, `종료 코드`, `로그 요약`)가 `WpfLabelingShellWindow.YoloEnvironmentRuntimeCommands.cs`에 직접 남아 있어, 실행 workflow와 사용자-facing 문구가 섞여 있었습니다.
- 수정 내용:
  - `WpfYoloEnvironmentCommandPresentationService`에 `WpfUltralyticsPackageConfirmationPresentation` DTO와 Ultralytics 설치/제거 문구 생성 메서드를 추가했습니다.
  - 설치/제거 가능 여부 안내, 취소 상태, 진행 상태, 시작 로그, 성공/실패 상태, 확인 팝업, 결과 상세 생성을 presentation service로 옮겼습니다.
  - `ConfirmUltralyticsPackageOperation`은 service가 만든 DTO를 `WpfMessageDialog.Confirm`에 전달하는 UI adapter만 맡습니다.
  - `WpfLabelingShellWindow.YoloEnvironmentRuntimeCommands.cs`의 `BuildUltralyticsPackageOperationDetail`과 log-first-line helper를 제거했습니다.
  - `--wpf-yolo-model-settings-panel` 테스트가 service의 확인 팝업/결과 상세 생성과 shell inline 문구 금지를 검증하도록 갱신했습니다.
- 검증:
  - `dotnet build .\tests\LabelingApplication.Tests\LabelingApplication.Tests.csproj -c Debug /nr:false -m:1 /p:UseSharedCompilation=false /p:OutDir=artifacts\isolated-out\` 통과, 경고 0 / 오류 0.
  - `dotnet .\tests\LabelingApplication.Tests\artifacts\isolated-out\LabelingApplication.Tests.dll --wpf-yolo-model-settings-panel` 통과.
  - `dotnet .\tests\LabelingApplication.Tests\artifacts\isolated-out\LabelingApplication.Tests.dll --wpf-labeling-shell` 통과.
