...
```text
1 데이터셋 -> 2 라벨링 -> 3 AI 후보 검토(선택) -> 4 학습/모델 -> 모델 적용 -> 다시 검사
```
...
| AI 후보 검토(선택) | 현재 검사 모델로 자동 라벨 후보를 만들 때만 사용합니다. | 직접 라벨링만 했다면 건너뛰고 학습/모델로 이동 |
...
패널을 펼치면 제목 바로 아래의 탭으로 현재 단계의 세부 화면을 바꿉니다.
...
2. 저장 라벨 목록, 현재 작업, 클래스 정보를 확인해야 하면 왼쪽 작업 패널을 연 뒤, 제목 아래의 `라벨`, `작업`, `클래스` 탭을 선택합니다.
...
객체탐지와 세그멘테이션에서는 저장 여부와 별도로 품질 검수 상태를 기록합니다.

| 품질 검수 | 의미 |
| --- | --- |
| 미검토 | 라벨은 있을 수 있지만 아직 사람이 품질을 확인하지 않은 상태 |
| 수정 필요 | 라벨 오류를 발견해 다시 손봐야 하는 상태. 이미지 큐의 `수정 필요` 필터로 모아 볼 수 있음 |
| 검수 완료 | 저장된 라벨을 사람이 확인해 현재 기준으로 승인한 상태 |

`검수 완료`는 라벨 저장이 끝난 이미지에서만 선택할 수 있습니다. 검수 완료 뒤 라벨을 다시 편집하면 상태는 `미검토`로 돌아가며, `수정 필요` 상태는 수정 후 저장해도 작업자가 다시 판정할 때까지 유지됩니다.

수정 사유를 한 줄로 입력한 뒤 `수정 필요`를 누르면 상태와 사유가 함께 저장됩니다. `QA 보고서`는 현재 이미지별 검수 집계와 수정 필요 목록을 저장 폴더의 `label-quality-review.md`로 내보냅니다.
...
객체탐지 레시피에 YOLOv5와 YOLOv8 실행기/가중치가 모두 등록돼 있으면 `v5 vs v8 분석`으로 같은 분할의 정확도와 모델 Takt 보고서를 만들 수 있습니다. 보고서는 `학습/비교`의 `성능 비교 열기`에서 확인합니다. `test`가 없을 때 표시되는 `val` 결과는 엔진 성능 참고용이며 검사 모델 교체 근거가 아닙니다.

## 11. 현재 검사와 후보 검토

AI 후보 검토는 자동 라벨 후보를 사용할 때만 거치는 선택 단계입니다.
직접 라벨링을 마쳤다면 건너뛰고 `4 학습/모델`로 이동합니다.
사용할 때는 현재 검사 모델로 후보를 만들고, 맞는 후보만 저장 라벨로 넘깁니다.

![AI 후보 검토 패널](images/annotated/12-inference-dock-1920-annotated.png)

작업 순서:

1. 자동 라벨 후보가 필요하면 `3 AI 후보 검토(선택)`로 이동합니다.
2. `현재 검사`를 실행합니다.
3. 후보가 있으면 위치와 클래스를 봅니다.
   세그멘테이션 후보는 내부 채움이나 사각형 없이 검출 외곽선으로, 객체검출 후보는 박스로 표시됩니다.
4. 맞으면 확정합니다.
5. 틀리면 스킵합니다.
6. 확정된 후보는 저장 라벨이 되므로 마지막에 라벨 저장을 누릅니다.
7. 후보가 없으면 추가 조작 없이 `4 학습/모델`로 이동합니다.
...
`성능 비교 열기`는 별도 창에서 객체 탐지, 세그멘테이션, 이상 분류 실행을 모아 보여줍니다. 최대 6개 실행을 선택하고 그중 하나를 기준으로 지정할 수 있습니다. 작업 종류, 평가 데이터, 분할, 이미지 수 또는 Takt 측정 조건이 다르면 우열을 계산하지 않고 조건 차이를 먼저 표시합니다.
...
- 작업 필터로 같은 목적의 실행을 찾고 비교할 실행과 기준 모델을 선택합니다.
- 같은 test split으로 비교합니다.
- 같은 클래스 목록과 라벨 기준으로 비교합니다.
- precision, recall, 놓침, 과검출을 같이 봅니다.
- 모델 Takt는 같은 이미지 크기, 배치, 반복 조건에서만 비교합니다.
- 이상 분류처럼 Takt가 기록되지 않은 결과는 정확도 근거만 확인합니다.
