...
```text
1 데이터셋 -> 2 라벨링 -> 3 추론 검토 -> 4 학습/모델 -> 모델 적용 -> 다시 검사
```
...
| 추론 검토 | 현재 검사 모델로 후보를 만들고 검토합니다. | 후보를 확정한 뒤 라벨 저장 |
...
| 번호 | 위치 | 역할 |
| --- | --- | --- |
| 1 | 작업 단계 | 데이터셋, 라벨링, 추론 검토, 학습/모델 이동 |
| 2 | 이미지 큐 | 이미지별 저장 상태, 후보 상태, 실패 상태 확인 |
| 3 | 캔버스 | 라벨과 후보 위치 확인 |
| 4 | 작업 패널 | 현재 단계에서 필요한 설정과 검토 |
| 5 | 검사 모델 | 지금 검사에 쓰는 모델 확인 |

화면이 복잡해 보이면 먼저 상단 작업 단계와 왼쪽 이미지 큐만 봅니다.
